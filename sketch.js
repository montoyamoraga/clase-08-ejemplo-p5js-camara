...
function setup() {
  ...
  // crear nuevo stream desde la camara
  camara = createCapture(VIDEO);

  // ocultar stream original
  camara.hide();
...
function draw() {
  ...
  // mapear datos recibidos desde perilla [0,1023] a porcentaje [0, 1]
  let tamano = map(datosSeparados[1], 0, 1023, 0, 1);;


  // mapear datos recibidos desde perilla [0,1023] a rango color [0, 1]
  let colorPotenciometro = map(datosSeparados[1], 0, 1023, 0, 1);
  ...
  // si el pulsador está presionado
    ...
    // poner stream en el lienzo, tamano ajustado segun potenciometro
    image(camara, 0, 0, tamano * width, tamano * width * camara.height / camara.width);
